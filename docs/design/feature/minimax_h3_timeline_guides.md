...
`recipes/MiniMaxAI/MiniMax-H3.md`. The offline interface is in
`examples/offline_inference/minimax_h3/end2end.py`.
...
| Offline adapter | `examples/offline_inference/minimax_h3/end2end.py` | Repeatable local-file `--guide` arguments |
...
| `tests/examples/offline_inference/test_minimax_h3_guides.py` | CLI ordering, local path validation and sampling transport |
...
combinations passed. Tests and temporary scripts under `/tmp/kilo` are local
