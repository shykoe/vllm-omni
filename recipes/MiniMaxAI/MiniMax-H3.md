...
The existing offline example accepts repeatable `--guide FRAME SOURCE...`
arguments. Unlike the HTTP manifest, these are trusted local-file inputs from
the operator. They use the same model-side normalization and limits:

```bash
python examples/offline_inference/minimax_h3/end2end.py \
  --model /path/to/MiniMax-H3/FL2VA --task t2va \
  --width 1344 --height 768 --num-frames 124 --fps 24 \
  --steps 50 --seed 1101 --quality lossless --cache-backend none \
  --diffusion-attention-backend FLASH_ATTN \
  --guide 36 image=middle.png \
  --guide 0 audio=rain.wav \
  --guide -22 video=tail_22_frames.mp4 audio=tail.flac \
  --prompts 'A white bird flies over the lake, then lands in the reeds.'
```

Select parallelism/offload for available hardware as in the existing offline
examples; do not launch this on occupied GPUs. Quote source arguments that
contain spaces, for example `"image=middle frame.png"`. Repeated flags preserve
guide order; no workflow JSON is required. Remove all `--guide` flags for the
same-seed no-guide comparison.
...
/data/miniconda3/envs/vllm_omni/bin/python -c "import vllm; print(vllm.__version__); from vllm_omni.diffusion.models.minimax_h3.pipeline_minimax_h3 import MiniMaxH3Pipeline"
```

Preflight found four idle NVIDIA H20 GPUs (97,871 MiB each) and about 2.1 TiB
available host RAM. `/data/MiniMax-H3/FL2VA/model_index.json` was readable;
...
python -m pytest -q tests/examples/offline_inference/test_minimax_h3_guides.py
```

The default Python and the project environment did not contain pytest. The
first two commands were retried with isolated pytest tools using the prefix
`env PYTHONPATH=/tmp/kilo/guide01-test-tools:/tmp/kilo/guide01-api-test-tools /data/miniconda3/envs/vllm_omni/bin/python`
in place of `python`; collection then hit the dependency errors above. The
offline CLI suite passed 24 tests with the isolated tools. The packing suite
...
```bash
env PYTHONPATH=/tmp/kilo/guide01-test-tools /data/miniconda3/envs/vllm_omni/bin/python -m pytest -q tests/diffusion/models/minimax_h3/test_minimax_h3_timeline_guides.py
env PYTHONPATH=/tmp/kilo/guide01-test-tools /data/miniconda3/envs/vllm_omni/bin/python -m pytest -q tests/examples/offline_inference/test_minimax_h3_guides.py
env PYTHONPATH=/tmp/kilo/guide01-test-tools /data/miniconda3/envs/vllm_omni/bin/python -c "import sys, types, pytest; p='vllm_omni.diffusion.models.minimax_h3'; m=types.ModuleType(p); m.__path__=['vllm_omni/diffusion/models/minimax_h3']; sys.modules[p]=m; raise SystemExit(pytest.main(['-q','tests/diffusion/models/minimax_h3/test_minimax_h3_packing.py']))"
```

An additional guarded run passed **341 tests** across H3 contract, step,
stage-adapter, packing, offload, and parallel suites. Its local harness
`/tmp/kilo/guide01_pipeline_tests.py` replaces only the unavailable paged-KV
layout import with fail-if-used guards; reaching that unsupported implementation
fails the test rather than simulating success. Tests use CPU tensors and mocked
model components, including simulated two-rank post-encode failures. They do not
prove recovery from process loss or hangs inside native collectives.

```bash
env PYTHONPATH=/data/vllm-omni:/tmp/kilo/guide01-test-tools:/tmp/kilo/guide01-api-test-tools /data/miniconda3/envs/vllm_omni/bin/python /tmp/kilo/guide01_pipeline_tests.py -q --tb=short tests/diffusion/models/minimax_h3/test_minimax_h3_contract.py tests/diffusion/models/minimax_h3/test_minimax_h3_step_execution.py tests/model_executor/stage_input_processors/test_minimax_h3.py tests/diffusion/models/minimax_h3/test_minimax_h3_packing.py tests/diffusion/models/minimax_h3/test_minimax_h3_offload.py tests/diffusion/models/minimax_h3/test_minimax_h3_parallel.py
```
...
```bash
env PYTHONPATH=/tmp/kilo/guide01-test-tools:/tmp/kilo/guide01-api-test-tools /data/miniconda3/envs/vllm_omni/bin/python -m pytest -q tests/entrypoints/openai_api/test_video_guided_lifetime.py
env PYTHONPATH=/tmp/kilo/guide01-test-tools:/tmp/kilo/guide01-api-test-tools /data/miniconda3/envs/vllm_omni/bin/python -m pytest -q tests/engine/test_orchestrator_error_handling.py --tb=short
```
...
```bash
env PYTHONPATH=/data/vllm-omni:/tmp/kilo/guide01-test-tools:/tmp/kilo/guide01-api-test-tools /data/miniconda3/envs/vllm_omni/bin/python /tmp/kilo/guide01_pipeline_tests.py -q --tb=short tests/diffusion/models/minimax_h3/test_minimax_h3_timeline_guides.py tests/diffusion/models/minimax_h3/test_minimax_h3_contract.py tests/diffusion/models/minimax_h3/test_minimax_h3_step_execution.py tests/model_executor/stage_input_processors/test_minimax_h3.py tests/diffusion/models/minimax_h3/test_minimax_h3_packing.py tests/diffusion/models/minimax_h3/test_minimax_h3_offload.py tests/diffusion/models/minimax_h3/test_minimax_h3_parallel.py tests/examples/offline_inference/test_minimax_h3_guides.py tests/entrypoints/openai_api/test_video_guided_lifetime.py tests/engine/test_orchestrator_error_handling.py
```

### Installed-media follow-up (2026-09-14)

After ffmpeg and ffprobe were installed in `/data/miniconda3/envs/vllm_omni/bin`,
...
```bash
env PATH=/data/miniconda3/envs/vllm_omni/bin:$PATH PYTHONPATH=/tmp/kilo/guide01-test-tools:/tmp/kilo/guide01-api-test-tools /data/miniconda3/envs/vllm_omni/bin/python -m pytest -q tests/diffusion/models/minimax_h3/test_minimax_h3_timeline_guides.py --tb=short
env PATH=/data/miniconda3/envs/vllm_omni/bin:$PATH PYTHONPATH=/data/vllm-omni:/tmp/kilo/guide01-test-tools:/tmp/kilo/guide01-api-test-tools /data/miniconda3/envs/vllm_omni/bin/python /tmp/kilo/guide01_pipeline_tests.py -q --tb=short --basetemp=/tmp/kilo/guide01-installed-media-tests tests/diffusion/models/minimax_h3/test_minimax_h3_timeline_guides.py tests/diffusion/models/minimax_h3/test_minimax_h3_contract.py tests/diffusion/models/minimax_h3/test_minimax_h3_step_execution.py tests/model_executor/stage_input_processors/test_minimax_h3.py tests/diffusion/models/minimax_h3/test_minimax_h3_packing.py tests/diffusion/models/minimax_h3/test_minimax_h3_offload.py tests/diffusion/models/minimax_h3/test_minimax_h3_parallel.py tests/examples/offline_inference/test_minimax_h3_guides.py tests/entrypoints/openai_api/test_video_guided_lifetime.py tests/engine/test_orchestrator_error_handling.py
```

Fixtures remain under `/tmp/kilo/guide01-installed-media-tests/`. The actual
...
the guarded test harness:

```bash
env PATH=/data/miniconda3/envs/vllm_omni/bin:$PATH CUDA_VISIBLE_DEVICES=0,1,2,3 /data/miniconda3/envs/vllm_omni/bin/python examples/offline_inference/minimax_h3/end2end.py --model /data/MiniMax-H3/FL2VA --task t2va --width 1344 --height 768 --num-frames 124 --fps 24 --steps 50 --seed 1101 --quality lossless --cache-backend none --diffusion-attention-backend FLASH_ATTN --usp 4 --enforce-eager --init-timeout 60 --guide -22 video=/tmp/kilo/guide01-installed-media-tests/test_real_video_probe_and_deco5/guide.mp4 --prompts "A quiet cinematic landscape with a bird flying over a lake." --output /tmp/kilo/guide01-installed-media-tests/model-output
```
...
```bash
env LD_LIBRARY_PATH=/data/miniconda3/envs/vllm_omni/lib/python3.13/site-packages/nvidia/cudnn/lib:$LD_LIBRARY_PATH /data/miniconda3/envs/vllm_omni/bin/python -c "import torch; print('cuDNN',torch.backends.cudnn.version()); x=torch.zeros((1,4,16,16),device='cuda'); y=torch.nn.Conv2d(4,4,3).cuda()(x); torch.cuda.synchronize(); print('CUDA convolution OK',tuple(y.shape))"
```
...
Artifact directory: `/tmp/kilo/guide01-installed-media-tests/vllm029-tail/`.
...
```bash
env PATH=/data/miniconda3/envs/vllm_omni/bin:$PATH LD_LIBRARY_PATH=/data/miniconda3/envs/vllm_omni/lib/python3.13/site-packages/nvidia/cudnn/lib:$LD_LIBRARY_PATH CUDA_VISIBLE_DEVICES=0,1,2,3 /data/miniconda3/envs/vllm_omni/bin/python examples/offline_inference/minimax_h3/end2end.py --model /data/MiniMax-H3/FL2VA --task t2va --width 1344 --height 768 --num-frames 124 --fps 24 --steps 50 --seed 1101 --quality lossless --cache-backend none --diffusion-attention-backend FLASH_ATTN --usp 4 --text-encoder-tp-size 4 --vae-patch-parallel-size 4 --vae-use-tiling --enforce-eager --init-timeout 600 --guide -22 video=/tmp/kilo/guide01-installed-media-tests/test_real_video_probe_and_deco5/guide.mp4 --prompts "A quiet cinematic landscape with a bird flying over a lake." --output /tmp/kilo/guide01-installed-media-tests/vllm029-tail
```
...
```bash
/data/miniconda3/envs/vllm_omni/bin/ffmpeg -v error -i /tmp/kilo/guide01-installed-media-tests/vllm029-tail/minimax_h3_t2va_0.mp4 -vf 'select=eq(n\,36)' -frames:v 1 /tmp/kilo/guide01-installed-media-tests/vllm029-tail/middle.png
/data/miniconda3/envs/vllm_omni/bin/ffmpeg -v error -f lavfi -i sine=frequency=440:sample_rate=32000:duration=1 -ac 2 /tmp/kilo/guide01-installed-media-tests/vllm029-tail/tone.flac
curl --noproxy 127.0.0.1 --fail-with-body --silent --show-error http://127.0.0.1:8079/v1/videos -F 'prompt=A quiet cinematic landscape with a soft electronic tone.' -F 'width=1344' -F 'height=768' -F 'num_frames=124' -F 'fps=24' -F 'num_inference_steps=50' -F 'seed=1101' -F 'quality=lossless' -F 'extra_params={"task":"t2va","aspect_ratio":"16:9","preencode_mp4":true}' -F 'timeline_guides=[{"frame_index":0,"audio":{"upload_index":0}}]' -F 'guide_files=@/tmp/kilo/guide01-installed-media-tests/vllm029-tail/tone.flac;type=audio/flac'
curl --noproxy 127.0.0.1 --fail-with-body --silent --show-error --max-time 1800 http://127.0.0.1:8079/v1/videos/sync -F 'prompt=A quiet cinematic landscape with a bird flying over a lake.' -F 'width=1344' -F 'height=768' -F 'num_frames=124' -F 'fps=24' -F 'num_inference_steps=50' -F 'seed=1101' -F 'quality=lossless' -F 'extra_params={"task":"t2va","aspect_ratio":"16:9"}' -F 'timeline_guides=[{"frame_index":36,"image":{"upload_index":0},"audio":{"upload_index":1}}]' -F 'guide_files=@/tmp/kilo/guide01-installed-media-tests/vllm029-tail/middle.png;type=image/png' -F 'guide_files=@/tmp/kilo/guide01-installed-media-tests/vllm029-tail/tone.flac;type=audio/flac' --output /tmp/kilo/guide01-installed-media-tests/vllm029-tail/http-sync-av.mp4
curl --noproxy 127.0.0.1 --fail-with-body --silent --show-error --max-time 1800 http://127.0.0.1:8079/v1/videos/sync -F 'prompt=A quiet cinematic landscape with a bird flying over a lake.' -F 'width=1344' -F 'height=768' -F 'num_frames=124' -F 'fps=24' -F 'num_inference_steps=50' -F 'seed=1101' -F 'quality=lossless' -F 'extra_params={"task":"t2va","aspect_ratio":"16:9"}' --output /tmp/kilo/guide01-installed-media-tests/vllm029-tail/http-sync-baseline.mp4
```
...
with `env PATH=/data/miniconda3/envs/vllm_omni/bin:$PATH PYTHONPATH=/data/vllm-omni /data/miniconda3/envs/vllm_omni/bin/python /tmp/kilo/guide01_analyze_outputs.py`.
...
The request/error records are in `http-error-propagation.json`. The executable
check is:

```bash
/data/miniconda3/envs/vllm_omni/bin/python /tmp/kilo/guide01_validate_http_errors.py
```
...
```bash
curl --noproxy 127.0.0.1 --fail-with-body --silent --show-error --max-time 300 http://127.0.0.1:8079/v1/videos/sync -F 'prompt=A quiet cinematic landscape with a bird flying over a lake.' -F 'width=1344' -F 'height=768' -F 'num_frames=124' -F 'fps=24' -F 'num_inference_steps=3' -F 'seed=1101' -F 'quality=lossless' -F 'extra_params={"task":"t2va","aspect_ratio":"16:9"}' -F 'timeline_guides=[{"frame_index":-1,"image":{"upload_index":0}}]' -F 'guide_files=@/tmp/kilo/guide01-installed-media-tests/vllm029-tail/middle.png;type=image/png' --output /tmp/kilo/guide01-installed-media-tests/vllm029-tail/http-recovery-last-image-3sigma.mp4
```
...
```bash
env PATH=/data/miniconda3/envs/vllm_omni/bin:$PATH \
  LD_LIBRARY_PATH=/data/miniconda3/envs/vllm_omni/lib/python3.13/site-packages/nvidia/cudnn/lib:$LD_LIBRARY_PATH \
  PYTHONPATH=/tmp/kilo/guide01-test-tools:/tmp/kilo/guide01-api-test-tools \
  /data/miniconda3/envs/vllm_omni/bin/python -m pytest -q --tb=short \
  ...
  tests/examples/offline_inference/test_minimax_h3_guides.py \
